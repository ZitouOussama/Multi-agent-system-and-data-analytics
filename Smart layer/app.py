...
@app.route('/')
def index():
    return 'Welcome to Flutter API'

@app.route('/api/v1/scrapNews', methods=['GET']) 
def startScraping():
    """ scrap n number of pages news """
    numOfPages = 50 #request.args['numOfPages']
    scrap_moroccan_news(numOfPages)

    return jsonify("news scraped successfuly") , 200

@app.route('/api/v1/scrapCovData', methods=['GET']) 
def doScraping():
    """ scrap covid 19 data """
    typeData = "predictions" #request.args['typeData']
